        ...
        public static string CurrentLanguage
        {
            ...
            set
            {
                if (value != null && value.Trim()!= string.Empty)
                {
                    ...
                    currentLocalizationText = Resources.Load(STR_LOCALIZATION_PREFIX + currentLanguage, typeof(TextAsset)) as TextAsset;//забираем инфу из заготовленного текста
                    if (currentLocalizationText == null)
                    {
                        Debug.LogWarningFormat("Missing locale ’0’,loading English.", currentLanguage);
                        currentLanguage = SystemLanguage.English.ToString();//заменяет язык на английский
                        Debug.Log("Произошла замена");
