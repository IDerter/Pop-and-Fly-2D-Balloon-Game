...
public class RewardedAds : SingletonBase<RewardedAds>//, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    [Tooltip("Ассет покупки, который отключает принудительную рекламу (Даст награду моментально)")]
    ...
    {
        YG2.onRewardAdv += OnYandexReward;
        YG2.onCloseRewardedAdv += OnYandexClosed;
        YG2.onErrorRewardedAdv += OnYandexError; // Добавляем событие ошибки
    }
    ...
    private void OnDisable()
    {
        YG2.onRewardAdv -= OnYandexReward;
        YG2.onCloseRewardedAdv -= OnYandexClosed;
        YG2.onErrorRewardedAdv -= OnYandexError;
    }

    private void OnYandexError()
    {
        Debug.LogWarning("[RewardedAds] Yandex rewarded ad failed to show (Error).");
        _isRewardPending = false; // Сбрасываем флаг, чтобы можно было нажать кнопку еще раз
    }

    private void OnYandexReward(string id)
    {
        // Проверяем, что награда пришла именно за тот тип рекламы, который мы запрашивали
        if (id == _type.ToString()) 
        {
            OnRewardedAdReward();
        }
    }
    ...
    private void OnYandexClosed()
    {
        OnRewardedAdClosed();
    }

    // Добавь этот метод (перегрузку), чтобы не ломать старые вызовы
    public void ShowRewardedAd(TypeReward type, GameObject toDelete, System.Action onComplete)
    {
        _pendingRewardCallback = onComplete; // Сохраняем логику, которая придет из UI
        ShowRewardedAd(type, toDelete);      // Вызываем твой старый метод
    }
    ...
    public void ShowRewardedAd(TypeReward type, GameObject toDelete = null)
    {
        if (_isRewardPending)
        {
        ...
        // --- ПРОВЕРКА НА ПОКУПКУ (ВЫДАЕМ МОМЕНТАЛЬНО) ---
        if (_removeAdsUpgrade != null && Upgrades.GetUpgradeLevel(_removeAdsUpgrade) > 0)
        {
            Debug.Log("[RewardedAds] Игрок купил 'No Ads'. Выдаем награду моментально без видео!");
        ...
        if (YG2.platform == "YandexGames")
        {
            ...
            // Просто ставим флаг и вызываем рекламу мгновенно
            _isRewardPending = true;
